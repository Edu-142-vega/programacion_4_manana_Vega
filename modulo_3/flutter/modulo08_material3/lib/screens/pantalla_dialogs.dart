...
class PantallaDialogs extends StatelessWidget {
  ...
  void _mostrarSnackBar2(BuildContext context, {bool esError = false}) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(esError
            ? 'Error: no se pudo conectar al servidor ERROR'
            : 'CONEXION ESTABLECIDA (VIVSTE)'),
        backgroundColor: esError
            ? Theme.of(context).colorScheme.error
            : null,
        action: SnackBarAction(
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          Text('SNACKBAR MIOOOO', style: text.labelLarge?.copyWith(color: cs.primary)),
          const SizedBox(height: 12),
          FilledButton.icon(
            onPressed: () => _mostrarSnackBar2(context),
            icon:  const Icon(Icons.check_circle_outline),
            label: const Text('SnackBar de EXITACION'),
          ),
          ...
          Text('MI ALERTA ', style: text.labelLarge?.copyWith(color: cs.primary)),
          const SizedBox(height: 12),
          OutlinedButton.icon(
            style: OutlinedButton.styleFrom(
              foregroundColor: cs.error,
            ...
            onPressed: () => _mostrarConfirmacion(context),
            icon:  const Icon(Icons.delete_outline),
            label: const Text('EliminaRSE DE LA VIDA (confirmación)'),
          ),
        ],
      ),
    );
