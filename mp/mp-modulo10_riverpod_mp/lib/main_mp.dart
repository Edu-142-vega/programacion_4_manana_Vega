...
void main() {
  runApp(const ProviderScope(child: AppExpedientesRiverpod()));
}

class AppExpedientesRiverpod extends StatelessWidget {
  const AppExpedientesRiverpod({super.key});
...
class _Paso1 extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Expedientes Policiales Activos')),
      body: Center(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text('$count', style: Theme.of(context).textTheme.displayLarge),
            const Text('Expedientes en Investigación'),
          ],
        ),
      ),
